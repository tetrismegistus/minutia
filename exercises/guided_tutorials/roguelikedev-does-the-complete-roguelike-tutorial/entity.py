...
class Entity:
    ...
    def move_astar(self, target, entities, game_map):
        ...
        # rooms) if for example the player is in a corridor.  It makes sense to keep the path size relatively low to
        # keep monsters from running around the map if there's an alternative path really far away
        if not libtcod.path_is_empty(my_path) and libtcod.path_size(my_path) < 25:
            ...
            if x or y:
                ...
                # Keep the old move function as a backup so that if the there are no paths (for example another monster
